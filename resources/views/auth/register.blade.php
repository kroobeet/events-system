<x-guest-layout>
    <form method="POST" action="{{ route('user-register') }}">
        ...
        <div>
            ...
            <x-text-input id="birth_year" class="block mt-1 w-full" type="text" name="birth_year" :value="old('birth_year')" autofocus autocomplete="birth_year" />
        ...
        <div class="mt-4">
            ...
            <select name="role" id="role" class="block mt-1 w-full" required>
                @foreach($roles as $role)
                    <option value="{{ $role->name }}">{{ $role->comment }}</option>
                @endforeach
            </select>
        </div>
